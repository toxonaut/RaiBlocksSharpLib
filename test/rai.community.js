 ...
 httpsgithub.comSergiySWRaiBlocksJS

 Released under the BSD 3-Clause License


 RaiBlocks Community website
 httpsraiblockscommunity.net
...
function RaiCommunity() {
	...
	this.account = function(account) {
		var account = this.json(httpsraiblockscommunity.netaccountindex.phpacc= + account + &json=1, JSON.stringify({}));
		return account;
	}

	this.block = function(hash) {
		var block = this.json(httpsraiblockscommunity.netblockindex.phph= + hash + &json=1, JSON.stringify({}));
		return block;
	}
	
	
	this.frontiers = function() {
		var frontiers = this.json(httpsraiblockscommunity.netpagefrontiers.phpjson=1, JSON.stringify({}));
		return frontiers;
	}
	...
	this.peers = function() {
		var peers = this.json(httpsraiblockscommunity.netpagepeers.phpjson=1, JSON.stringify({}));
		return peers;
	}
	
	
	this.representatives = function() {
		var representatives = this.json(httpsraiblockscommunity.netpagerepresentatives.phpjson=1, JSON.stringify({}));
		return representatives;
	}
	
	
	this.summary = function() {
		var summary = this.json(httpsraiblockscommunity.netpagesummary.phpjson=1, JSON.stringify({}));
	...
	this.transaction_boost = function(frontier, boost_count) {
		...
		$.each(chain, function(){
			...
			let url = httpsraiblockscommunity.netprocessblockelaborate.php;
			let params = processblock= + url_block + &submit=Process;
			http.open(POST, url, true);
			http.setRequestHeader(Content-type, applicationx-www-form-urlencoded);
	...
	this.transaction_booster = function(wallet, host) {
		if (typeof host == 'undefined') { host = 'httplocalhost7076'; }  if not initialized
		var rai = new Rai(host);
		var community_request = this;
